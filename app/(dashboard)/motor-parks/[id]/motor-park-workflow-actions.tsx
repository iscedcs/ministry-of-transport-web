...
export function MotorParkWorkflowActions({
  ...
  const handlePsApprove = () => {
    startTransition(async () => {
      ...
      const res = await psApproveMotorPark(parkId, levyValue, psNotes);
      if (res.success) {
          ...
          <CardContent className="space-y-4">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="space-y-1.5">
                <Label htmlFor="monthly-levy" className="text-xs font-semibold">
              ...
              <div className="space-y-1.5">
                <Label htmlFor="ps-notes" className="text-xs font-semibold">
        ...
        <Card className="border-emerald-500/30 bg-emerald-500/5">
          <CardHeader className="pb-3">
            <CardTitle className="text-base text-emerald-900 dark:text-emerald-200">
              Hon. Commissioner Final Sign-off & Permit Issuance
            </CardTitle>
            <CardDescription>
              Review complete dossier & Permanent Secretary recommendation, then give final approval to append digital signature and issue permit.
            </CardDescription>
          </CardHeader>
          <CardContent className="flex flex-wrap items-center gap-3">
            <Button asChild className="bg-emerald-600 hover:bg-emerald-700 text-white font-semibold">
              <Link href={`/motor-parks/${parkId}/issue-permit`}>
                <CheckCircle2 className="w-4 h-4 mr-2" /> Approve & Issue Permit to Build
              </Link>
            </Button>
            <Button asChild variant="outline" className="border-emerald-600 text-emerald-700 dark:text-emerald-300 font-semibold">
